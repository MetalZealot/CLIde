...
export function createProviderRuntimeService(
  ...
  return {
     ...
     *
     * Runtimes share a single pending-request registry, so at most one of them
     * can consume a given requestId — every other runtime answers `not_found`,
     * which is why `resolved` short-circuits and outranks everything else.
     */
    async resolveInteractiveRequest(
      requestId: string,
      response: InteractiveRequestResponse,
    ): Promise<ProviderInteractiveResolution> {
      let aggregate: ProviderInteractiveResolution = { status: 'not_found' };

      for (const provider of dependencies.listProviders()) {
        const permissions = provider.runtime.permissions;
        if (!permissions) {
          continue;
        }

        const result = await permissions.resolve(requestId, response);
        if (result?.status === 'resolved') {
          return result;
        }
        if (result?.status === 'invalid' && aggregate.status === 'not_found') {
          aggregate = result;
        }
      }
