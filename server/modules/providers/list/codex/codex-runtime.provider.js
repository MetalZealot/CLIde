...
function extractCodexTokenBudget(event) {
  ...
  return extractCodexContextTokenUsage(
    info,
    event?.usage?.total_token_usage || event?.usage,
  );
}
...
const defaultCodexRuntimeContext = {
  ...
  resolveResumeModel: (sessionId, requestedModel) =>
    providerModelsService.resolveResumeModel('codex', sessionId, requestedModel),
  getProviderModels: async () => (await providerModelsService.getProviderModels('codex')).models,
  normalizeMessage: (raw, sessionId) => sessionsService.normalizeMessage('codex', raw, sessionId),
 ...
 * @param {object} context - Registry-supplied runtime context
 */
export async function queryCodexSdk(command, options = {}, ws, context = defaultCodexRuntimeContext) {
  const {
    sessionId,
    sessionSummary,
    cwd,
