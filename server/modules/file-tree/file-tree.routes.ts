...
import { normalizeSourcePathsInput } from '@/modules/projects/index.js';
import type {
  FileTreeLogger,
  FileTreeServices,
...
export function createFileTreeRouter(
  ...
  // Move one or more files/directories into another directory. Accepts
  // `sourcePaths: string[]`; the original singular `sourcePath` still works so
  // an older client keeps functioning.
  router.put('/projects/:projectId/files/move', createRouteHandler(async (request, response) => {
    const body = readBody(request);
    const sourcePaths = normalizeSourcePathsInput(body);
    const { moved, skipped } = await services.moveEntries({
      projectId: readProjectId(request),
      sourcePaths,
      destinationPath: readRequiredString(body.destinationPath, 'destinationPath'),
    });
