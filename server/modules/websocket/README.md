...
flowchart TD
  ...
  D -->|chat.steer| J[capability check -> providerRuntimeService.steer]
...
5. `chat.steer` is request-correlated and accepted only while the session has a
running turn whose effective runtime advertises active-turn steering. It emits
`chat_input_accepted` or `chat_input_rejected` without changing run ownership.
