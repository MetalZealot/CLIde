...
test('usage command keeps /cost working as an alias', async () => {
  const context = { provider: 'claude', tokenUsage: { used: 1200, total: 200000 } };

  const usage = await executeCommand('/usage', context);
  const cost = await executeCommand('/cost', context);

  assert.equal(usage.action, 'usage');
  assert.equal(cost.action, 'usage');
  assert.deepEqual(cost.data, usage.data);
});

test('usage command reports session tokens under the usage action', async () => {
  const result = await executeCommand('/usage', {
    provider: 'claude',
    tokenUsage: {
      used: 1200,
      total: 200000,
  ...
  assert.equal(data.tokenUsage.used, 1500);
  assert.equal(data.tokenUsage.total, 200000);
  assert.deepEqual(data.tokenBreakdown, { input: 1000, output: 500 });
  assert.equal(data.provider, 'claude');
});
