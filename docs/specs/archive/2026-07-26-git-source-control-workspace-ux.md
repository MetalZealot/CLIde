...
- Related reference: `docs/specs/2026-07-25-agentic-coding-ux-reference.md`
...
Rescoped 2026-08-05. The original draft inventoried everything a mature IDE's
source-control surface contains, which made a reference document read like a
plan and gave a forty-feature backlog to a single maintainer.

The split now is explicit:

- **The main body is what CLIde is building.** If it is here, it is in scope.
- **"Git mental model" builds nothing.** It is background that makes the rest
  legible. It is the longest section and creates no obligation whatsoever.
- **Appendix A is recorded but not planned.** Real IDE features that CLIde has
  decided not to pursue for now, kept so they are not rediscovered from scratch
  and re-argued. Moving something out of the appendix is a deliberate act.

## The point

Everything here serves one capability that conventional IDEs do not have,
because they were not built for it:

> **Several agents are working in parallel worktrees of one repository, and I
> can see them together — which directory each is editing, which branch it has
> checked out, and when more than one of them is loose in the same working
> tree.**

VS Code, JetBrains, and Zed all manage worktrees competently. None of them has
a reason to show *agent occupancy*, because their worktrees hold one human
each. CLIde's do not. When two threads share a checkout they share one working
tree and their edits can collide, and the one-project-per-directory view CLIde
inherited hides that completely.

That is the differentiator. Truthful Git state, a worktree inventory, and
creation flows are all in scope because occupancy is meaningless without them —
not because a source-control panel is expected to have them.
...
3. **"Group threads under the project, not under branch names" is not a
   ...
   repository-grouping layer specified below.

## Purpose

CLIde has a capable Git panel, but its model is still mostly "the current
directory plus a branch picker." That is insufficient for a product where
people and agents routinely use several linked worktrees, where the app may be
serving one of those worktrees, and where a fork commonly has both an `origin`
and an `upstream`.
...
history, and current-branch push/pull. Its gaps are truthfulness and workspace
lifecycle:
...
Make existing state truthful, then add a worktree inventory and occupancy, and
only then add guarded integration operations.

---

# Git mental model

*Reference only. Nothing in this section is a feature request.*
...
That explains CLIde's self-hosting footgun: when the selected project is the
checkout from which CLIde itself is running, a branch switch rewrites the
...
directory, `HEAD`, staging index, uncommitted changes, and in-progress
operation state such as a merge or rebase.
...
| --- | --- | --- |
| OS account | the host login | Local files, processes, and home directory |
...
rename the OS account, change the GitHub login, alter SSH authorization, or
rewrite old commits. GitHub primarily associates command-line commits with an
account through the commit email. This caused a real, understandable confusion:
commits authored as the OS username were already correctly attributed by GitHub
to `MetalZealot`, because the email was associated with that account.

---

# Projects, repositories, and checkouts in CLIde

This is the load-bearing section. Everything else depends on getting these
three objects apart.
...
2. **A linked worktree becomes a separate top-level project.** Running
   ...
   unrelated in the UI to the project it was branched from.

That second consequence is the gap this document exists to close.

## Target model

**One project, containing every checkout of its repository, with each
checkout's branch, dirty state, and agent occupancy visible in one place** — so
concurrent agents working in parallel worktrees are observable together rather
than scattered across unrelated sidebar entries.
...
Project  (organisational container, what the user names and opens)
  └─ Repository  (identified by `git rev-parse --git-common-dir`)
       ...
       ├─ Checkout: worktree        <repo>-wt-topic
       │    branch feat/topic · unpublished · 8 changes · 2 agents active
       └─ Checkout: worktree        <repo>-wt-exp
...
- **A checkout can hold several threads.** When it does, say so, because those
  agents share one working tree and their edits can collide.
...
Questions settled during implementation (ADR 0016): a worktree registers as an
ordinary project row and is absorbed into its repository's row by the grouping
layer; renaming and archiving apply to the individual checkout, while delete
covers the repository because the row *is* the repository.
...
- `upstream/main` is sufficient as the clean upstream reference.
...
history. The remote preserves shared/published history.

---

# Truth and safety gaps in the current implementation

## What is already strong

Working-tree status grouped by modified/added/deleted/untracked; a real staged
files model; stage and unstage; file diffs; commit and initial-commit flows;
recent history with topology and ref decorations; local-versus-remote branch
sections; local branch creation and safe `git branch -d`; current-branch
tracking status; fetch, pull, push, and first-publish; explicit confirmations
for several consequential actions.
...
**This is the prerequisite for everything else.** A panel that silently swallows
errors cannot host guarded destructive operations: every refusal, preflight
failure, and conflict report specified here depends on the UI being able to
show a server-side error at all.
...
- **Conflicts are deliberately folded into `modified`.** `parseGitStatusOutput`
  detects `U`/`AA`/`DD` entries and then pushes them onto `modified` so they can
  never appear staged. The intent is sound, but conflict state is consequently
  not representable in the API at all — a conflict count cannot be displayed
  because the data never leaves the server.
...
- **Detached `HEAD` renders as a branch named `HEAD`.** `getCurrentBranchName`
  tries `git symbolic-ref --short HEAD` and falls back to
...
`/api/git/branches` strips `remotes/<remote>/` from every ref and deduplicates
by basename. The client consequently cannot distinguish `origin/main` from
`upstream/main`, represent branches that exist on several remotes, or safely
create a tracking branch from the selected remote ref. The API should return
structured refs rather than parallel string arrays.

This is an upstream defect, not a CLIde one — see `docs/upstream-candidates.md`.

## Checkout lacks workspace preflight

The checkout route validates the branch name and runs `git checkout <branch>`.
It provides no structured checks for dirty files that will move or block, the
intended local branch for a remote ref, a branch checked out in another
worktree, detached `HEAD`, merge/rebase state, or the checkout being used to
serve CLIde.

---

# What CLIde is building

## Occupancy

The headline capability. A checkout's occupancy is **application state layered
onto Git state, never inferred from branch names**, and CLIde already knows all
of it: it runs from a known checkout, the dev and branch-test services have
fixed queryable workspaces, and sessions carry `project_path`.

Show, per checkout:

- how many agent threads are bound to it;
- when that number is greater than one, that they share a working tree and can
  collide — this is a warning, not a statistic;
- whether it serves the CLIde runtime, a dev server, or a branch-test service.

`sessions.project_path` makes the thread count a query, not a new subsystem.

## Persistent workspace status

The always-visible status area should answer, without opening a menu:
repository name; active worktree, especially when it is not the main one;
current branch or detached `HEAD`; clean/dirty/conflicted/merging/rebasing
state; tracking branch; incoming/outgoing counts; unpublished state; and
...
Status is passive information. Clicking the branch name can open the
workspace/branch picker, but must not perform a switch by itself.

On mobile it collapses to two lines and stays readable:
...
occupancy — rather than several small unrelated popovers. Per ADR 0005, the
status line is passive and tapping it must never perform a switch.

## Repository/worktree selector

Worktrees appear as workspace entries under their shared repository:

```text
CLIde
├─ main                 <repo>                    clean · serving
├─ docs/git-source...   <repo>-wt-git-...         1 modified
└─ feat/example         <repo>-wt-example         clean · agent active
```

Each entry shows: main versus linked worktree; path; branch or detached commit;
clean/dirty/conflict/locked/prunable state; last activity; ahead/behind against
that branch's tracking ref; occupancy; and actions appropriate to its state.

Selecting a worktree changes CLIde's selected workspace. It must not secretly
switch the branch of the current directory.

## Operation state in the changes view

The conventional changes view (conflicts, staged, unstaged, untracked, per-file
diff, stage/unstage/discard, commit) already exists. What it needs added is
explicit operation state above the file groups:

- merge/rebase/cherry-pick in progress;
- source and target refs;
- continue, abort, or open conflict resolver.

Destructive file actions must state whether they affect the index, working
tree, untracked files, or history.

## Worktree creation flow

Required fields: source repository; base branch or commit; new or existing
local branch; worktree location; optional bootstrap profile.

Preflight must detect: branch already checked out in another worktree;
destination already exists or is nested unsafely; invalid branch name; ignored
files/dependencies that will not appear automatically.

After creation, CLIde should open/select the new worktree, be able to start a
session in it, and leave the original workspace unchanged.
...
Default refusal conditions: dirty or conflicted worktree; untracked files;
locked worktree; branch contains commits not integrated into the selected
comparison target; active agent, terminal, dev server, test service, or CLIde
runtime uses it; main worktree or currently selected workspace.

The confirmation must state the directory to be removed, whether the branch
will be retained, whether all commits are reachable from another named ref, and
...
Bringing a finished topic branch back is step 5 of the fork workflow above, so
it stays in scope — but as a named operation, not a wizard.

**Name the actual operation.** Applying a patch, cherry-picking commits, and
merging a branch produce different history and different conflict behaviour.
CLIde must never offer an unqualified **Apply**:
...
**State the direction**, defaulting source to the active topic branch and
target to the mainline, with both visible:
...
**Preflight** reports source and target commits, merge base, commits unique to
each side, files changed, whether the target can fast-forward, dirty or
conflicted worktrees, where each branch is checked out, occupancy, and whether
the source is already integrated.

**Prefer fast-forward**, then an explicit merge commit when histories diverged
and the user approves. Rebase and cherry-pick are Appendix A.

**Execute in the target worktree.** Git merges into the currently checked-out
target, so CLIde must resolve that worktree and refuse if it is dirty or has an
operation in progress. If the target serves CLIde or a dev process, block or
disclose that the source tree will change.

**Report cleanup as separate remaining actions** — verification, push, worktree
removal, branch deletion. No "Finish" action performs them silently.

## Agent threads and checkouts

A thread's target is the checkout it edits, recorded on the thread rather than
inferred from whatever project is selected later — which CLIde already does.
...
## Commit identity in Settings

Settings must separate commit identity from authentication, because they are
different systems (see the four-identity table above). Show the effective
author name and email, the scope and source (repository, global, system, or
environment override), and state plainly that it affects future commits only.
Offer repository-specific override and global default as distinct choices.

The full multi-credential status inspector is Appendix A; this is the part that
resolves the actual confusion.
...
- full ref names are canonical API identities; short names are display only;
- worktree IDs are not inferred from mutable paths in the client;
...
- occupancy is application state, not guessed from branch names.

Use Git's porcelain formats intended for machines, including
...
| Repository | *did not exist* | `git rev-parse --git-common-dir`, cached per project — **implemented** |
| Checkout kind | *does not exist* | main vs linked, from `git worktree list --porcelain` |
| Occupancy | *does not exist* | application state — serving runtime, dev server, branch-test service, active sessions |

---

# Guardrail policy

**Read-only** — status, log, diff, branch listing, worktree listing, remotes,
fetch preview, merge preview. No confirmation. Fetch changes remote-tracking
refs but not working files, so it can be one click with visible progress and
errors.
...
deletion. Require explicit targets. Never expose an unqualified force push.

**Self-hosting protection** — CLIde must know the repository/worktree containing
its runtime source and any registered dev/test service workspaces. Actions that
rewrite those files are blocked or elevated with precise consequences. A
dirty-tree warning alone does not cover runtime disruption.

---

# Implementation sequence

## Phase 0: make what already exists truthful

Prerequisite for everything else. Each item is small and independently
shippable, and several are upstreamable.
...
## Phase 1: identity and grouping — **implemented**

Repository identity derived from `--git-common-dir`, projects grouped by it,
and the sidebar presenting one row per repository with sessions merged across
its checkouts and labelled by branch. See ADR 0016 and
`server/modules/projects/services/repository-identity.service.ts`.
...
1. Show shared-checkout thread counts so concurrent agents are visible. ← *the point*
2. Add structured worktree discovery from `git worktree list --porcelain`. The
   sidebar still infers a repository's checkouts from registered projects, so a
   worktree created outside CLIde is invisible until someone starts a session
   in it.
3. ~~Create a linked worktree from a selected base and new branch.~~ **Done**
   (`worktree.service.ts`, `POST /:projectId/worktrees`, the sidebar's worktree
   manager). Shipping this ahead of Phase 0 is recorded in ADR 0028; removal in
   item 6 is not covered by that and still waits.
4. ~~Open/select the resulting workspace.~~ **Done** — creation selects the new
   checkout and closes the manager.
5. ~~Report Git failures separately from bootstrap/registration failures.~~
   **Done.** The destination is preflighted against the workspace root before
   git runs, so the common orphan case cannot happen; if registration fails
   anyway the response still reports success with a `registrationError`, and
   the UI names the path so the directory is findable.
6. Safe worktree removal, retaining the branch by default. **Blocked on Phase 0
   by ADR 0028** — this is the destructive half.

Items 3–5 have static coverage only (`worktree.service.test.ts`); none of it is
live-verified.

## Phase 3: integration

1. Source/target selection and merge preview.
2. Fast-forward integration.
3. Explicit merge commits with conflict reporting.
4. Verification, push, worktree cleanup, and branch cleanup reported as
...
Replacing the original thirteen-question audit, which was a coordination tool
for a team. The single acceptance test:

> **On a phone, without opening a terminal, I can see that CLIde has three
> checkouts, which branch each one is on, and that two agent threads are live
> in the same directory — and I can create a fourth worktree from a chosen base
> and start a session in it.**

The supporting product contract:
...
- [x] Project, repository, checkout, branch, and thread are distinct objects.
- [x] Every checkout of one repository is visible in one place.
- [x] Threads remain bound to their checkout unless an explicit handoff occurs.
- [x] Detached `HEAD` is displayed truthfully in the sidebar, with a short SHA.
- [x] Branches and checkouts have separate icons and separate actions.
- [ ] Shared-checkout occupancy is visible when several threads share a directory.
- [ ] Detached `HEAD` is displayed truthfully **in the Git panel**.
- [ ] Server-side failures are visible in the UI.
- [ ] Staged, unstaged, untracked, and conflicted states are separately inspectable.
- [ ] In-progress merge/rebase/cherry-pick states are prominent.
...
# Appendix A: recorded, not planned

Real features of mature IDE source-control surfaces that CLIde is **not**
building. They are kept here so they are not rediscovered and re-argued from
scratch, and so the main body stays a plan rather than an inventory.

Moving an item out of this appendix is a deliberate decision, not a natural
next step. Each is individually a multi-week project maintained by a team at
Microsoft or JetBrains.

**History and comparison graph.** All local and remote refs with namespaces,
current `HEAD`, branch and tag decorations, merge topology, incoming/outgoing
commits, "compare with current worktree", "create branch/worktree from this
commit", commit details and changed files. *Why deferred:* a commit graph with
merge topology is table stakes on a desktop IDE and near-useless on a phone,
which is CLIde's primary form factor.

**Full branch and remote browser.** Refs grouped by remote with per-branch
actions: switch, open existing worktree, create branch from ref, compare,
merge into current, rename, publish to a selected remote, delete local with
merged status, delete remote as a separate explicit action. *Why deferred:* the
truthfulness half (preserve remote namespaces, show occupancy, offer "Open
Worktree" not "Switch") is in Phase 1; the full browser is not.

**Remote configuration inspector.** Fetch URL, push URL, default behaviour,
last fetch, authentication method, fetch/prune controls, and per-remote fetch.

**Multi-credential authentication status.** Independently tested status for Git
transport (SSH/HTTPS), the `gh` API account and scopes, and a CLIde GitHub
integration token, each with last successful validation and a **Reconnect**
action, with errors naming the failed layer ("SSH authentication to
`github.com` failed", not "GitHub authentication expired"). *Why deferred:* the
commit-identity half is in scope and resolves the real confusion; the rest is a
credential-management product.

**Pull requests and collaboration.** PR state, checks, reviews, branch
protection visibility.

**Advanced history rewriting.** Rebase with explicit rewrite consequences,
cherry-pick, squash, force-with-lease.

**Stash/shelve workflow.**

**Three-way conflict editor.** Conflict *reporting* is in Phase 0; a merge
editor is not.

**Per-turn change review.** A result card summarising the changes attributable
to one turn:
...
*Why deferred, and the warning worth keeping:* repository status alone cannot
reconstruct turn attribution — that needs snapshot boundaries. **Before
building any Git-snapshot subsystem, check what CLIde's rewind support already
records** (`server/modules/providers/list/claude/claude-rewind.util.ts`,
`src/components/chat/view/subcomponents/RewindEditCard.tsx`,
`server/modules/providers/services/provider-capabilities.service.ts`).
Claude Code exposes a native rewind
concept with checkpoint-like boundaries, and a parallel snapshot system would
duplicate it. Rewind is Claude-specific, so per the multi-provider rule any
turn-scoped diff must be gated on a declared capability and degrade to hidden
for Cursor, Codex, and OpenCode.

**Execution environments.** Explicitly out of scope, not merely deferred — see
...
The original acceptance criteria, kept as an audit instrument rather than a
target. A finished surface should let a developer answer these from the UI
without knowing Git's command syntax:
