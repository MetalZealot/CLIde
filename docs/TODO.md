...
- [~] **Codex rollout verification: the live acceptance matrix.** `npm run test:codex` is green (314 tests, 0.152.1). Left: run the live rows the bump requires — Runtime identity first, since daily use never exercises it. [Conformance map](maps/codex-integration-conformance.md). **M**
...
- [ ] **Archiving a session you just finished does not stick.** `createSession` clears `isArchived` on both write paths, and the full rescan re-indexes every recently created transcript on each project-list fetch. Port upstream `bfe7c495`'s `CASE WHEN` guard and its two tests. Careful tier: back up `auth.db` first. [Plan](plans/upstream-1373-sync.md). **S**
- [ ] **A failed server build destroys the running one.** `prebuild:server` deletes `dist-server` before `tsc` runs, so an out-of-memory build leaves no entrypoint and the app crash-loops until rebuilt by hand. Upstream `0d517749` stages into `dist-server.next` and promotes after `tsc-alias`. [Plan](plans/upstream-1373-sync.md). **S**
