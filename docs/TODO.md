...
- [~] **Replace CLIde's home-grown Browser MCP with official Playwright MCP.** Phases 0-5 done; Claude and Codex drive the tools live. Cursor and OpenCode are not installed here, so their registrations stay config-verified. Open: acceptance, then retirement. [Plan](plans/browser-mcp-hardening.md), [ADR 0053](decisions/0053-browser-tools-are-official-playwright-mcp-over-http.md). **L**
- [~] **Browser viewport sizes are configurable per preset.** Settings > Browser gains a Viewport group: each of Desktop, Tablet and Mobile picks a named Playwright device or a custom size, stored in `browser_use_settings` and read when a context is leased. A custom size on a touch preset keeps that preset's user agent, pixel density and touch. Awaiting acceptance on 3002. **S**
