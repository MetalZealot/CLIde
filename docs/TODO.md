...
- [~] **Codex rollout verification: the live acceptance matrix.** The 0.153.4 gate, Astra default/new Chat, async Send now/Queue priority, and rewind lineage passed in isolation. Broader rows remain, with diagnostics/runtime identity first because daily use does not exercise it. [Conformance map](maps/codex-integration-conformance.md). **M**
