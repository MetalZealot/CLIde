...
- [~] **Codex rollout verification: the live acceptance matrix.** `npm run test:codex` is green (99 tests, 0.149.1). Left: re-run at 0.152.1, now installed and pinned, then run the rows the bump requires — Runtime identity first, since daily use never exercises it. [Conformance map](maps/codex-integration-conformance.md). **M**
...
- [~] **Skills Settings combines every saved project/worktree into one provider list.** Provider discovery and the one-target client request are corrected; remaining work is the Context picker and isolated acceptance. [Plan](plans/skills-settings-discovery-scope.md). **S/M**
