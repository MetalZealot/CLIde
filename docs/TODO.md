...
- [~] **Browser sessions get a policy in Settings.** Settings > Browser gains a Sessions group (starting preset, concurrent-session cap, idle timeout with a Never option) and an Allowed sites group (allow and block origin lists passed to the Playwright MCP connection). The two limits were environment-only; the origin lists were not reachable at all. Awaiting acceptance on 3002. **S**
