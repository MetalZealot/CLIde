...
- [~] **Usage dashboard and reset notifications.** Routed account view per authenticated provider, honest unsupported states, fresh windows and balances, default-off reset alerts. All six phases done, accepted live on `:3002` 2026-08-17 after rebasing onto SDK 0.3.233. Left: merge, an SSH `test:client` run, and one observed reset. [Plan](plans/usage-dashboard.md). **L**
