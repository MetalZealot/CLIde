...
- [~] **Codex Chat runs bundled 0.146 while Shell runs native 0.147.** The `isBlocking` protocol gap is closed; next pin the bundled SDK/CLI to 0.147, then make the native runtime selected and promoted rather than whatever is on `PATH`. [Plan](plans/codex-0-147-managed-native-runtime.md). **L**
