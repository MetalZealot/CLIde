...
- [ ] **`resolveClaudeTranscriptPath` misses the runtime's 200-char branch.** Since runtime 2.1.224 the encoded project directory is `slice(0,200) + '-' + base36(hash(path))` once it exceeds 200 chars; CLIde derives only the plain form, so a long workspace path resolves to the wrong directory. Reachable only when a session row has no `jsonlPath`. [Ledger](maps/claude-upgrade-ledger.md). **S**
