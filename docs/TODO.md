...
- [~] **Recurring provider SDK/CLI update process.** Cadence: [the maps README](maps/README.md); history in both ledgers. Landed: the registry-drift test (now reads the runtime binary), the version-pair recorder, Codex's single pinned literal. Open: a `check:providers` script diffing installed-vs-latest; Cursor/OpenCode maps; unknown-method diagnostics. **M recurring**
