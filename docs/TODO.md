...
- [~] **Auto-Continue offered, remembered, and defaulted.** Every way of asking for a `usage-reset` message that is not long-pressing send and typing "Continue": the one-tap offer on the limit notice is live; an editable continue message and a per-session standing mode with a firing cap are left. Reuses the shipped scheduler entirely. [Plan](plans/auto-continue.md). **M**
