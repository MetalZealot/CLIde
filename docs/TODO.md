...
- [~] **Codex Chat's bundled SDK/CLI is updated to 0.147 and live-verified on 3002.** The account card now checks and explicitly selects path-distinct installations; next align the runtime decisions and maps with reality. [Plan](plans/codex-0-147-managed-native-runtime.md). **L**
- [ ] **The Codex runtime row never says which installation Check and Use act on.** `CodexNativeRuntimeRow.tsx` derives one candidate — first non-active, non-bundled — and the buttons target it silently; correct with today's two installations, ambiguous the moment a third is discovered. Make the target row explicit. Same row, mobile: full paths wrap to three lines each. **S**
