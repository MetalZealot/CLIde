...
- [~] **Usage dashboard and reset notifications.** Add a routed account-level view for every authenticated provider, with honest unsupported states, fresh plan windows and balances, collapsed detail, and default-off per-provider reset alerts through enabled system channels. [Plan](plans/usage-dashboard.md). **L**
