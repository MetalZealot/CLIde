...
- [~] **Claude Agent SDK bumped 0.3.165 → 0.3.233**, and the three runtime-gated items with it. Awaiting one live Chat session with a rewind on 3001 after an SSH restart. The premise that this moved Chat's runtime was wrong — CLIde always sets `pathToClaudeCodeExecutable`, so the bundled binary is a fallback and Chat already ran the `PATH` runtime. [Ledger](maps/claude-upgrade-ledger.md). **M**
