...
- [~] **Recurring upstream-fork sync process.** `npm run check:upstream` reports the span and which commits are ruled on; verdicts and the ledger live in [the map](maps/upstream-sync.md). [Plan](plans/upstream-1373-sync.md). Open: `0d517749` unassessed as of 2026-09-09. **M recurring**
...
- [~] **Chat history loading, scrolling and Find remain slow.** Phases 1–6 complete: server reuse, unchanged-message rendering, stable pagination, bounded payloads and Find without rendered history. Next: mounted-content limits, after the tool-activity plan's collapsed rows. [Plan](plans/chat-history-performance.md), [evidence](maps/chat-history-performance.md). **L**
...
- [~] **Chat renders raw tool calls instead of activities.** A burst of 14 reads/commands is 14 cards; measured, prose-bounded clustering takes 295 calls to 51 rows. Claude and Codex first. Phases 1–4 merged: one muted row per burst, one line per call when open; not yet seen on a phone. [Plan](plans/tool-activity-display.md), [map](maps/tool-activity-stream.md). **L**
- [ ] **Codex history shows edits as raw `exec` source.** An apply_patch, `write_stdin` or `web__run` call reloads as an untranslated `exec` row, not `FileChanges`; the activity row parses it client-side, but the open card still shows source text. Translate it in the Codex adapter. [Map](maps/tool-activity-stream.md). **S**
...
- [~] **Double-tap Esc to stop mid-send and immediately edit.** The stop-and-recover half landed (`e5ede32` + `adab285`, ADR 0013). The Esc gesture landed too (`d6a3e980`): first Escape or tap arms Stop, second fires. Remaining: editing a turn the provider *did* take, which is a rewind, not a retraction. **M**
