...
- [~] **Codex's Agent screen is decluttered.** Merged `88543d1`; awaiting live verification. Runtime and chat transport moved off the account card to a registry-driven `agent.codex.runtime` sub-screen; the card now shows only a degraded-transport alert. One state badge per installation, rollback on the previous install's own row, every API enum mapped to a sentence. **S**
