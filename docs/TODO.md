...
- [~] **Build the upstream capabilities worth having.** Nine verdicts are recorded; next, Grayson ranks the three "build" verdicts, then each gets a design. [Plan](plans/upstream-feature-harvest.md). **L — waiting on Grayson**
...
- [~] **Subagents are invisible while they run and unreadable after.** Phases 1, 2 and 4 landed; next, watch `subagents/**` so a running agent updates without a reload, then open its whole transcript in place. Never re-index them as sessions. [Plan](plans/subagent-visibility.md). **M/L**
