...
- [~] **Self-hosted dictation and read-aloud.** Core voice behavior passes on phone. The eight selected voices and tiered Settings picker are implemented with HFC Male as the initial choice and model-native speed; live picker acceptance and Rocket Raccoon's licence remain in Phase 5. Bluetooth mic diagnosis is separate. [Plan](plans/self-hosted-voice.md). **L**
