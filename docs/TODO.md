...
- [~] **Self-hosted dictation and read-aloud.** Core voice behavior and the Settings picker pass across sessions. Spike is replaced by US-medium Kusal; accepted HFC, Rocket, and Jenny pacing presets are restored. Kusal, HFC repeated zeroes, and Rocket's licence remain in Phase 5. Bluetooth mic diagnosis is separate. [Plan](plans/self-hosted-voice.md). **L**
