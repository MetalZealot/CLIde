...
- [~] **Sidebar hierarchy pass** — in progress on `feat/sidebar-hierarchy`. One section-label metric instead of three (Pinned 16px / Projects 44px / Sessions 24px), one row rhythm, a real 12px nesting indent on a rail aligned to the accent strip, one metadata size, project titles at 500, and New Session moved to the mobile footer as an in-flow pill. **M**
