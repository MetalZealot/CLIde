...
- [~] **Replace CLIde's home-grown Browser MCP with official Playwright MCP.** Phases 0-5 done; Phase 6 driven live and passing on the topic server. Open: uncapped screenshot bytes, `fullPage` action timeout, output dirs left by an unclean shutdown, plus acceptance. [Plan](plans/browser-mcp-hardening.md), [ADR 0053](decisions/0053-browser-tools-are-official-playwright-mcp-over-http.md). **L**
