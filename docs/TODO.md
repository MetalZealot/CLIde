...
- [~] **Chat renders raw tool calls instead of activities.** A burst of 14 reads/commands is 14 cards; measured, prose-bounded clustering takes 295 calls to 51 rows. Claude and Codex first. Phases 1–5, 7–8 merged: one row per burst, one line per call, each call flat in place; thinking, questions, agents match. [Plan](plans/tool-activity-display.md), [map](maps/tool-activity-stream.md). **L**
