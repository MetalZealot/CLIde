...
- [~] **Replace CLIde's home-grown Browser MCP with official Playwright MCP while retaining the monitored Browser tab.** Phases 0-5 done: all four providers hold the HTTP registration and the stdio bridge is gone. Live acceptance is Phase 6. [Plan](plans/browser-mcp-hardening.md), [ADR 0053](decisions/0053-browser-tools-are-official-playwright-mcp-over-http.md). **L**
