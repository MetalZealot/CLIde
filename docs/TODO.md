...
- [~] **The Browser tab needs a layout pass.** Done at phone width: the card header splits into a title/URL row and a controls row, Stop uses a stop icon separated from Delete, and the panel subtitle hides below `sm`. Built and serving on the topic server; awaiting Grayson's look. **M**
