...
- [~] **Chat renders raw tool calls instead of activities.** A burst of 14 reads/commands is 14 cards; measured, prose-bounded clustering takes 295 calls to 51 rows. Claude and Codex first. Phases 1–5 and 7 merged: one muted row per burst, one line per call, a call's detail flat in place; thinking, questions match. [Plan](plans/tool-activity-display.md), [map](maps/tool-activity-stream.md). **L**
