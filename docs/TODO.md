...
- [~] **Chat renders raw tool calls instead of activities.** A burst of 14 reads/commands is 14 cards; measured, prose-bounded clustering takes 295 calls to 51 rows. Phases 1–5, 7–10 merged: one row per burst, one line per call, each call flat in place; thinking, questions, agents, plans, approvals match. [Plan](plans/tool-activity-display.md), [map](maps/tool-activity-stream.md). **L**
...
- [ ] **Workspace tabs like the desktop apps** — Terminal replaces Shell, plus Agents. [Plan](plans/workspace-surfaces.md). **M/L**
