...
- [~] **Composer menu follow-ups from 1.37.** Consolidate model/effort, limit permission cycling to safe routine modes with a complete picker, and simplify the action row; preserve ADR 0003/0025 precedence and keep `/models` as the command route to the same picker. **M/L** — live-accepted on `composer-edits`; awaiting merge
...
- [~] **The permission-mode picker's "Default" sends no mode**, silently inheriting `settings.json`. Mechanism, compounding layers, and fix options: [the permission map](maps/provider-permission-modes.md). The real deliverable is the terminology pass — "default" means three things across CLI, SDK, and picker. [upstreamable] **M** — live-accepted on `composer-edits`; awaiting merge
