...
- [~] **Auto-compact reads as two different ceilings.** The ring swaps between the SDK's `167,000 · Auto` and the derived, unlabelled `200,000` with nothing marking which is which, and `/autocompact 200000` is a no-op on a 200k model. [Plan](plans/autocompact-visibility.md). **M**
