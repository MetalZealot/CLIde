...
- [~] **Claude's (SDK, runtime) version pair is visible in Settings.** It was recorded on every auth check and dropped; it now reaches `ProviderAuthStatus.versions` and renders as one read-only Runtime row on the account card, with a seven-day notice naming whichever half moved. Awaiting a live look. [Ledger](maps/claude-upgrade-ledger.md). **S**
- [ ] **A usage reset notification has never been seen firing at a real provider reset.** Scheduling, coalescing and restart dedup are unit-covered, but no reset fell inside the test window, so delivery is unobserved. Enable the Claude toggle and watch the next five-hour boundary. [Archived plan](plans/archive/2026-08-17-usage-dashboard.md). **XS**
