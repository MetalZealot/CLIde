...
- [~] **Codex's Agent screen is decluttered.** Built, not yet committed or live-verified. Runtime and chat transport moved off the account card to a registry-driven `agent.codex.runtime` sub-screen; the card now shows only a degraded-transport alert. One state badge per installation, rollback on the previous install's own row, every API enum mapped to a sentence. **S**
...
- [ ] **Do forked subagents still mark rows `isSidechain`?** Default-on since runtime 2.1.232, and forks inherit the whole conversation; token usage, both rewind walkers and the session list assume those rows are skippable. Assess with one subagent run first. [Ledger](maps/claude-upgrade-ledger.md). **S**
- [ ] **Context ceiling ignores `CLAUDE_CODE_DISABLE_1M_CONTEXT`**, which runtime 2.1.223 made hold 1M models to 200K. Unset here; fix next time `claude-context-window.ts` is open. [Ledger](maps/claude-upgrade-ledger.md). **XS**
- [ ] **`resolveClaudeTranscriptPath` misses the runtime's 200-char branch.** Since 2.1.224 a long encoded project dir is `slice(0,200)-base36(hash)`, hash unidentified; CLIde derives only the plain form. Needs a row with no `jsonlPath` to bite. [Ledger](maps/claude-upgrade-ledger.md). **S**
...
- [~] **Session list decluttered and given batch actions** — Sessions subheader and its two controls deleted, sort/filter moved into the row's actions menu behind a "Filtered" cue, "Open in new tab" dropped, "Select…" adds per-repository batch Archive/Delete. Surface: [the sidebar map](maps/sidebar-surface.md). Awaiting live verification. **S**
...
- [x] **Three entry points for New Session, one of them scoped.** Resolved by deletion: the scoped per-row `+` is gone, leaving the header (desktop) and footer (mobile) — both global, so nothing needs distinguishing.
...
- [x] #3 — 1M-context alias mapping. Moot: the catalog no longer offers `[1m]` rows, and `resolveClaudeModelAlias` now folds a `[1m]` id onto its base model's row.
