...
- [~] **Chat actions in the mobile header kebab:** Pin, Rename (a dialog), Copy session ID, Export…, then Archive and Delete for the open session; Settings left the kebab, which now shows only on Chat and Shell; the floating export button is desktop-only. Built on `feat/mobile-bottom-navigation`; acceptance on the phone remains. [Plan](plans/mobile-bottom-navigation.md) **S**
