...
- [~] **Replace CLIde's home-grown Browser MCP with official Playwright MCP.** Phases 0-5 done; Phase 6 driven live on the topic server. Boot sweep, row pruning and the action timeout are fixed. Open: measure a `fullPage` shot, run it through each provider, acceptance. [Plan](plans/browser-mcp-hardening.md), [ADR 0053](decisions/0053-browser-tools-are-official-playwright-mcp-over-http.md). **L**
