...
- [~] **Replace CLIde's home-grown Browser MCP with official Playwright MCP.** Phases 0-5 done; Phase 6 driven live and passing on the topic server. Open: bound `fullPage` height and its timeout, sweep output dirs at boot, prune stopped panel rows, plus acceptance. [Plan](plans/browser-mcp-hardening.md), [ADR 0053](decisions/0053-browser-tools-are-official-playwright-mcp-over-http.md). **L**
