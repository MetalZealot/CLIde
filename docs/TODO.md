...
- [~] **Sidebar Activity section and collapsed-rail indicator.** Activity ordering/duplication, persistent search, footer Archive, rail status, and the mobile header close control are implemented per ADR 0030 on `cloudcli-wt-sidebar-activity-section`; awaiting follow-up acceptance and merge. **S**
