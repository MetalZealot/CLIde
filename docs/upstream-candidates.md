...
- [x] **Chat-history performance measurement harness. Fork-only tooling.** Synthetic fixtures, explicit pending targets and Browser/server baselines exercise CLIde's current contracts; no runtime fix is included. Upstream cache/rendering work already exists in inspected snapshot `5e73a49b`; adaptation and eligibility of later fixes stay in the [performance plan](plans/chat-history-performance.md).
