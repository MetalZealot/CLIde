...
import ThinkingMessagesEditor from './screens/ThinkingMessagesEditor';
...
describe('ThinkingMessagesEditor', () => {
  ...
  const blurTo = async (input: HTMLInputElement, relatedTarget: HTMLElement | null) => {
    await React.act(async () => {
      input.dispatchEvent(new window.FocusEvent('focusout', { bubbles: true, relatedTarget }));
    });
  };

  test('edits, reorders, adds, deletes, and resets messages with labelled controls', async () => {
    container = document.createElement('div');
    document.body.appendChild(container);
    root = createRoot(container);

    const Harness = () => {
      const defaults = ['Thinking', 'Processing'];
      const [messages, setMessages] = useState(defaults);
      const [cycleMode, setCycleMode] = useState<ThinkingMessageCycleMode>('4');
      const [messageOrder, setMessageOrder] = useState<ThinkingMessageOrder>('listed');
      ...
      return (
        <ThinkingMessagesEditor
          ...
          onCycleModeChange={(next) => {
            setCycleMode(next);
            setIsCustom(true);
          }}
          onMessageOrderChange={(next) => {
            setMessageOrder(next);
            setIsCustom(true);
          }}
          onReset={() => {
            setMessages(defaults);
            setCycleMode('4');
            setMessageOrder('listed');
            setIsCustom(false);
          }}
        />
      );
    ...
    const first = container.querySelector<HTMLInputElement>('[name="thinking-message-1"]');
    assert.ok(first);
    assert.equal(first.value, 'Thinking');
    const cycleSelect = container.querySelector<HTMLSelectElement>('[aria-label="Cycle messages"]');
    assert.ok(cycleSelect);
    assert.equal(cycleSelect.value, '4');
    assert.deepEqual(
      [...cycleSelect.options].map((option) => option.textContent),
      ['Never', 'Per turn', '2 seconds', '3 seconds', '4 seconds', '5 seconds'],
    );
    ...
    await React.act(async () => {
      ...
      valueSetter?.call(cycleSelect, 'never');
      cycleSelect.dispatchEvent(new window.Event('change', { bubbles: true }));
    });
    assert.match(container.textContent ?? '', /Only the first message is shown/);
    assert.equal(reset.disabled, false);
    const orderControl = container.querySelector<HTMLElement>('[role="radiogroup"][aria-label="Message order"]');
    assert.ok(orderControl);
    const [listedOrder, randomOrder] = orderControl.querySelectorAll<HTMLButtonElement>('[role="radio"]');
    assert.equal(listedOrder.textContent?.trim(), 'Listed');
    assert.equal(listedOrder.getAttribute('aria-checked'), 'true');
    assert.equal(listedOrder.disabled, true);
    assert.equal(randomOrder.textContent?.trim(), 'Random');
    assert.equal(randomOrder.disabled, true);

    await React.act(async () => reset.click());
    const resetDialog = document.querySelector<HTMLElement>('[role="dialog"]');
    assert.ok(resetDialog);
    assert.match(resetDialog.textContent ?? '', /Reset activity messages\?/);
    assert.match(resetDialog.textContent ?? '', /removes your custom messages/);
    assert.equal(cycleSelect.value, 'never', 'opening confirmation does not reset settings');
    const cancelReset = [...resetDialog.querySelectorAll<HTMLButtonElement>('button')]
      .find((button) => button.textContent?.trim() === 'Cancel');
    assert.ok(cancelReset);
    await React.act(async () => cancelReset.click());
    assert.equal(document.querySelector('[role="dialog"]'), null);
    assert.equal(cycleSelect.value, 'never');

    await React.act(async () => reset.click());
    const confirmReset = [...document.querySelectorAll<HTMLButtonElement>('button')]
      .find((button) => button.textContent?.trim() === 'Reset activity messages');
    assert.ok(confirmReset);
    await React.act(async () => confirmReset.click());
    assert.equal(cycleSelect.value, '4');
    assert.equal(reset.disabled, true);
    assert.equal(listedOrder.disabled, false);

    await React.act(async () => randomOrder.click());
    assert.equal(randomOrder.getAttribute('aria-checked'), 'true');
    assert.equal(reset.disabled, false);

    await changeInput(first, 'Pondering');
    assert.equal(first.value, 'Pondering');

    const actions = container.querySelector<HTMLButtonElement>('[aria-label="Actions for Pondering"]');
    assert.ok(actions);
    await React.act(async () => actions.click());
    const actionMenu = document.querySelector<HTMLElement>('[role="menu"]');
    assert.ok(actionMenu);
    assert.equal(actionMenu.getAttribute('aria-label'), 'Actions for Pondering');
    const [moveUp, moveDown, deleteAction] = actionMenu.querySelectorAll<HTMLButtonElement>('[role="menuitem"]');
    assert.equal(moveUp?.textContent?.trim(), 'Move up');
    assert.equal(moveUp?.disabled, true);
    assert.equal(moveDown?.textContent?.trim(), 'Move down');
    assert.equal(deleteAction?.textContent?.trim(), 'Delete');
    assert.ok(moveDown);
    await React.act(async () => moveDown.click());
    assert.equal(
      container.querySelector<HTMLInputElement>('[name="thinking-message-1"]')?.value,
      'Processing',
    );

    const newMessage = container.querySelector<HTMLInputElement>('[name="new-thinking-message"]');
    assert.ok(newMessage);
    await changeInput(newMessage, 'Considering');
    ...
    assert.equal(container.querySelectorAll('input[name^="thinking-message-"]').length, 3);

    const reorderedActions = container.querySelector<HTMLButtonElement>('[aria-label="Actions for Pondering"]');
    assert.ok(reorderedActions);
    await React.act(async () => reorderedActions.click());
    const remove = [...document.querySelectorAll<HTMLButtonElement>('[role="menuitem"]')]
      .find((button) => button.textContent?.trim() === 'Delete');
    assert.ok(remove);
    await React.act(async () => remove.click());
    assert.equal(container.querySelectorAll('input[name^="thinking-message-"]').length, 2);

    assert.equal(reset.disabled, false);
    await React.act(async () => reset.click());
    const finalConfirmReset = [...document.querySelectorAll<HTMLButtonElement>('button')]
      .find((button) => button.textContent?.trim() === 'Reset activity messages');
    assert.ok(finalConfirmReset);
    await React.act(async () => finalConfirmReset.click());
    assert.equal(reset.disabled, true);
    assert.equal(cycleSelect.value, '4');
    assert.equal(listedOrder.getAttribute('aria-checked'), 'true');
    assert.equal(
    ...
    const secondMessage = container.querySelector<HTMLInputElement>('[name="thinking-message-2"]');
    assert.ok(secondMessage);
    await changeInput(first, '   ');
    await blurTo(first, secondMessage);
    assert.equal(
      container.querySelectorAll('input[name^="thinking-message-"]').length,
      2,
      'an emptied row survives while the caret is still in the list',
    );

    await blurTo(first, null);
    assert.equal(container.querySelectorAll('input[name^="thinking-message-"]').length, 1);
    assert.equal(
      container.querySelector<HTMLInputElement>('[name="thinking-message-1"]')?.value,
      'Processing',
    );
  ...
  test('Chat links to the dedicated editor with the current message count', async () => {
    localStorage.removeItem(THINKING_MESSAGES_STORAGE_KEY);
    container = document.createElement('div');
    document.body.appendChild(container);
    root = createRoot(container);
    ...
    await React.act(async () => root?.render(
      <ChatScreen onOpenScreen={(screenId) => { openedScreen = screenId; }} />,
    ));

    const activityMessagesRow = [...container.querySelectorAll<HTMLButtonElement>('button')]
      .find((button) => button.textContent?.includes('Activity messages'));
    assert.ok(activityMessagesRow);
    assert.match(
      activityMessagesRow.textContent ?? '',
      /6 messages · Changes every 4 seconds/,
    );
    const summary = activityMessagesRow.querySelector('span.text-xs');
    assert.ok(summary);
    assert.equal(summary.classList.contains('whitespace-normal'), true);
    assert.equal(summary.classList.contains('truncate'), false);

    await React.act(async () => {
      localStorage.setItem(THINKING_MESSAGE_CYCLE_STORAGE_KEY, 'never');
      window.dispatchEvent(new window.StorageEvent('storage', {
        key: THINKING_MESSAGE_CYCLE_STORAGE_KEY,
        newValue: 'never',
      }));
    });
    assert.match(activityMessagesRow.textContent ?? '', /6 messages · First message only/);

    await React.act(async () => activityMessagesRow.click());
    assert.equal(openedScreen, 'chat.activityMessages');
  });
});
