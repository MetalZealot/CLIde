...
/**
 * Version, links, hosted CTA, Pro placeholders and license.
 *
 * The Pro placeholder cards and the hosted CTA are upstream's and stay behind
 * the existing `!IS_PLATFORM` guard rather than being removed, so this fork's
 * divergence here stays to chrome and keeps the rebase surface small.
 */
export default function AboutScreen() {
  const { t } = useTranslation('settings');
  const { updateAvailable, latestVersion, currentVersion, releaseInfo } = useVersionCheck('MetalZealot', 'CLIde');
  ...
  return (
    <SettingsScreen>
        ...
        <section className="space-y-3">
          ...
          <div className="space-y-4">
            <PremiumFeatureCard
              ...
              title={t('about.pro.sync.title')}
              description={t('about.pro.sync.description')}
            />
            <PremiumFeatureCard
              icon={<Users className="h-5 w-5" />}
              title={t('about.pro.team.title')}
              description={t('about.pro.team.description')}
            />
          </div>
        </section>
      )}
      ...
      {/* License */}
      <p className="border-t border-border/50 pt-4 text-xs text-muted-foreground/60">
        {t('about.license')}
      </p>
    </SettingsScreen>
  );
