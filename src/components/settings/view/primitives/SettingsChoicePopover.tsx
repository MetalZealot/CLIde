...
export default function SettingsChoicePopover<T extends string>({
  ...
  useEffect(() => {
    if (!isOpen) return;
    setActiveIndex(
      Math.min(selectedVisibleIndex, Math.max(0, visibleOptions.length - 1)),
    );
  }, [isOpen, query, selectedVisibleIndex, visibleOptions.length]);
  ...
  const handleListKeyDown = (event: React.KeyboardEvent<HTMLElement>) => {
    ...
    if (event.key === "Home" || event.key === "End") {
      event.preventDefault();
      setActiveIndex(
        event.key === "Home" ? 0 : Math.max(0, visibleOptions.length - 1),
      );
      return;
    }
  ...
  return (
    <>
      <button
        ...
        type="button"
        role="combobox"
        aria-label={[ariaLabel, selectedOption?.label, selectedOption?.detail]
          .filter(Boolean)
          .join(", ")}
      ...
      {isOpen &&
        createPortal(
          <div className="fixed inset-0 z-[9999]">
            ...
            <div
              ref={popoverRef}
              className="animate-in fade-in-0 zoom-in-95 absolute flex flex-col overflow-hidden rounded-lg border border-border bg-popover text-popover-foreground shadow-lg"
              style={{
              ...
              {searchable && (
                <div className="border-b border-border p-2">
                  <input
                    ...
                    className="w-full rounded-md border border-input bg-card px-3 py-2 text-base text-foreground placeholder:text-muted-foreground focus:border-primary focus:ring-1 focus:ring-primary"
                  />
                </div>
              )}
              <div
