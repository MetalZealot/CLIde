...
import QuestionPanelFrame from '../../tools/components/InteractiveRenderers/QuestionPanelFrame';
...
export default function AsyncQuestionPanel({
  ...
  return (
    <section
      ...
      className="mx-auto mb-3 max-w-[54.25rem]"
    >
      <QuestionPanelFrame
        waitingLabel={error
          ? t('followUpQuestions.answerNeedsAttention', { defaultValue: 'Answer needs attention' })
          : isSending
            ? t('followUpQuestions.sending', { defaultValue: 'Sending…' })
            : t('followUpQuestions.waiting', { defaultValue: 'Question waiting' })}
        heading={<>
        ...
        actions={
          <div className="flex w-full justify-end gap-2">
            <button
              ...
              className="min-h-8 rounded-md border border-border px-4 text-sm font-medium text-foreground transition-colors hover:bg-accent focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring disabled:cursor-not-allowed disabled:opacity-50"
            >
            ...
            <button
              ...
              className="min-h-8 rounded-md bg-primary px-4 text-sm font-medium text-primary-foreground transition-colors hover:bg-primary/90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring disabled:cursor-not-allowed disabled:opacity-50"
            >
              {isSending
                ? t('followUpQuestions.sending', { defaultValue: 'Sending…' })
                : isProcessing
      ...
      >
        <fieldset disabled={isSending} className="min-w-0 space-y-2.5 px-4 py-3">
          <legend id={`${groupName}-title`} className="float-left mb-2 w-full text-sm font-medium leading-snug text-foreground">
            {question.question}
          </legend>
